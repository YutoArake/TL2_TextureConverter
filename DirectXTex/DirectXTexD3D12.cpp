...
namespace
{
    ...
    inline void TransitionResource(
        _In_ ID3D12GraphicsCommandList* commandList_,
        ...
        _In_ D3D12_RESOURCE_STATES stateAfter)
    {
        ...
        commandList_->ResourceBarrier(1, &desc);
    }
        ...
        D3D12_RESOURCE_STATES afterState) noexcept
    {
        ...
        hr = device_->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAlloc.Get(), nullptr, IID_GRAPHICS_PPV_ARGS(commandList_.GetAddressOf()));
        if (FAILED(hr))
            return hr;

        // Create a fence
        ComPtr<ID3D12Fence> fence_;
        hr = device_->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_GRAPHICS_PPV_ARGS(fence_.GetAddressOf()));
        if (FAILED(hr))
            return hr;
            ...
            for (UINT plane = 0; plane < numberOfPlanes; ++plane)
            {
                for (UINT item = 0; item < desc.DepthOrArraySize; ++item)
                {
                    for (UINT level = 0; level < desc.MipLevels; ++level)
                    {
                        ...
                        commandList_->ResolveSubresource(pTemp.Get(), index, pSource, index, fmt);
                    }
                }
            }
        ...
        TransitionResource(commandList_.Get(), pSource, beforeState, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        for (UINT j = 0; j < numberOfResources; ++j)
        {
            ...
            commandList_->CopyTextureRegion(&copyDest, 0, 0, 0, &copySrc, nullptr);
        }

        // Transition the resource to the next state
        TransitionResource(commandList_.Get(), pSource, D3D12_RESOURCE_STATE_COPY_SOURCE, afterState);
        ...
        pCommandQ->ExecuteCommandLists(1, CommandListCast(commandList_.GetAddressOf()));

        // Signal the fence
        hr = pCommandQ->Signal(fence_.Get(), 1);
        if (FAILED(hr))
            return hr;

        // Block until the copy is complete
        while (fence_->GetCompletedValue() < 1)
        {
        #ifdef _WIN32
            SwitchToThread();
        #else
